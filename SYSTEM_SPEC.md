...
House Record는 임차인이 입주 및 퇴실 시점의 주거 공간 상태를 구조적으로 촬영하고, AI 촬영 감독과 해시 기반 로컬 블록체인 위변조 방지 등록을 통해 보증금 분쟁에 대비할 수 있도록 돕는 웹 애플리케이션이다.
...
- 입주 사진 세트는 사진 원본 대신 해시와 메타데이터 기반 evidence root로 로컬 체인에 등록된다.
- 퇴실 단계에서는 입주 기준 사진을 참고하여 동일 체크포인트를 다시 촬영하고 AI 비교 분석을 수행한다.
...
| `processing` | 블록체인 위변조 방지 등록 또는 AI 비교 분석 처리 중 |
| `report` | 블록체인 증명서, 검증 상태, AI 비교 결과 표시 |
| `album` | 입주 전 및 퇴실 당시의 촬영 사진 비교 대조 앨범 뷰 |
...
- 모든 공간의 체크포인트가 완료되면 evidence root를 로컬 체인에 위변조 방지 등록을 한다.
...
- 퇴실 촬영 완료 후 입주/퇴실 사진 비교 분석을 수행한다.
...
- 전체 점검: 모든 방이 완료되어야 블록체인 위변조 방지 등록 또는 퇴실 비교 분석 가능
...
입주 촬영 완료 후 시스템은 사진 세트와 메타데이터를 기반으로 evidence root를 생성한다.
...
증명서 화면에서는 현재 입주 사진 세트가 기존 evidence root와 일치하는지 검증한다.
...
- 동일 roomId/stepId의 입주 및 퇴실 첫 사진을 비교한다.
- 조명, 각도, 렌즈 왜곡은 무시하도록 프롬프트를 구성한다.
- 새 물리적 손상 여부를 `none`, `low`, `high`로 분류한다.

### F-014 단일 이미지 하자 분석

사용자는 별도 분석 화면에서 이미지를 업로드하여 단일 이미지 하자 분석을 요청할 수 있다.
...
사용자는 앨범 탭에서 입주 전과 퇴실 당시 촬영된 사진을 공간 및 체크포인트별로 병렬 대조할 수 있다.

#### 주요 기능
- 공간별 아코디언 접기/펼치기 제어
- 체크포인트별 입주 전 촬영 사진과 퇴실 당시 촬영 사진의 Side-by-Side (1:1) 비교
- AI 비교 분석 결과(Damage Level 및 분석 코멘트) 연동 표시
- 사진 미촬영 시 대체 플레이스홀더 제공

### F-016 서버 기반 사진 업로드 저장

브라우저 저장 공간의 한계와 초기화 가능성을 우회하기 위해, 촬영된 이미지를 로컬 개발 서버 및 클라우드 서버에 실제 파일로 업로드하여 URL 형태로 참조 및 저장한다.

#### 처리
- 클라이언트 캡처 시 base64 데이터를 `/api/upload` POST API로 송신
- 서버는 프로젝트 내부 `public/uploads` 디렉토리에 바이너리 파일(`.jpg`)로 물리적 저장
- 저장 완료 후 `/uploads/img_[timestamp]_[hash].jpg` 형태의 고유 경로 반환
- 클라이언트는 메타데이터 내에 사진 URL로 저장하여 IndexedDB 스토리지 오버헤드 최소화


### F-017 전체 점검 초기화 및 서버 리셋

전체 프로세스를 처음부터 깔끔하게 다시 시작할 수 있도록 로컬 데이터베이스와 서버의 업로드 에셋 파일을 일괄 삭제하는 초기화 기능을 제공한다.

#### 처리
- 사용자가 설정 바텀시트에서 "전체 점검 초기화" 실행 시 작동
- 클라이언트에서 서버 POST `/api/reset` API 호출
- 서버는 `public/uploads` 내의 모든 업로드 파일을 동기식으로 삭제(Purge)
- 삭제 성공 시 클라이언트는 `clearAppData()`를 수행하여 브라우저의 IndexedDB 데이터를 전부 청소하고, 최초 설정(`setup`) 상태로 새로고침(Reload) 진행
...
- 퇴실 비교 분석은 현재 동일 stepId의 첫 사진끼리 비교한다.
...
12. 모든 방 완료 후 블록체인 위변조 방지 등록
13. 증명서에서 root hash, txId, block hash, 검증 상태 확인
...
- 입주/퇴실 비교 시 모든 사진 조합 또는 대표 사진 선택 로직 개선
