...
- 입주 사진 세트는 사진 원본 대신 해시와 메타데이터 기반 evidence root로 로컬 체인에 고정된다.
...
| `processing` | 로컬 체인 앵커링 또는 AI 비교 분석 처리 중 |
| `report` | 로컬 체인 증거, 검증 상태, AI 비교 결과 표시 |
| `analyze` | 단일 이미지 업로드 하자 분석 |
...
- 전체 점검: 모든 방이 완료되어야 로컬 체인 앵커링 또는 퇴실 비교 분석 가능
...
보고서 화면에서는 현재 입주 사진 세트가 기존 evidence root와 일치하는지 검증한다.
